...
# Full load
rule1 = ctrl.Rule(
    (mClothes['full_load'] & dCloths['very_dirty']) |
    (mClothes['full_load'] & dCloths['medium_dirty']),
    wTime['very_long_time']
)
rule2 = ctrl.Rule(
    (mClothes['full_load'] & dCloths['lightly_dirty']) |
    (mClothes['full_load'] & dCloths['not_dirty']),
    wTime['little_time']
)

# Medium load
rule3 = ctrl.Rule(
    (mClothes['medium_load'] & dCloths['very_dirty']),
    wTime['very_long_time']
)
rule4 = ctrl.Rule(
    (mClothes['medium_load'] & dCloths['medium_dirty']) |
    (mClothes['medium_load'] & dCloths['lightly_dirty']),
    wTime['medium_time']
)
rule5 = ctrl.Rule(
    (mClothes['medium_load'] & dCloths['not_dirty']),
    wTime['little_time']
)

# Low load
rule6 = ctrl.Rule(
    (mClothes['low_load'] & dCloths['very_dirty']) |
    (mClothes['low_load'] & dCloths['medium_dirty']),
    wTime['long_time']
)
rule7 = ctrl.Rule(
    (mClothes['low_load'] & dCloths['lightly_dirty']) |
    (mClothes['low_load'] & dCloths['not_dirty']),
    wTime['little_time']
)
